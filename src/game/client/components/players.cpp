	...
	float Intra)
{
	...
	if(in_range(ClientId, MAX_CLIENTS - 1))
		...
		Position = GameClient()->MixToRenderSpace(
			CharacterNetPosX(&Prev), CharacterNetPosY(&Prev),
			CharacterNetPosX(&Player), CharacterNetPosY(&Player), Intra);
	...
	if(in_range(pPlayerChar->m_HookedPlayer, MAX_CLIENTS - 1))
		...
		HookPos = GameClient()->MixToRenderSpace(
			CharacterNetHookX(&Prev), CharacterNetHookY(&Prev),
			CharacterNetHookX(&Player), CharacterNetHookY(&Player), Intra);
